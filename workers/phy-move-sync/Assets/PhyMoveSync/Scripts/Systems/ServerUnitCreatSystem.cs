...
namespace PhyMoveSync
{
    ...
    public class ServerUnitCreatSystem : ComponentSystem
    {
        ...
        public ServerUnitCreatSystem(Vector3 origin)
        {
            workerOrigin = origin;
        ...
        protected override void OnUpdate()
        {
            foreach (var entityId in entitySystem.GetEntitiesAdded())
            {
                ...
                Debug.Log($"[{World.Name}] Entity {entityId} {metaData.EntityType} created.");

                var prefabPath = Path.Combine("Prefabs", $"{UnityGameLogicConnector.WorkerType}",
                    $"{metaData.EntityType}");

                UpdateServerEntity(entity, prefabPath, position, rotation);
            }
